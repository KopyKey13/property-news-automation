...
# Also check for any CSV files in the exports directory as a fallback
csv_path = None
if os.path.exists(csv_path_with_images):
    csv_path = csv_path_with_images
    print(f"Using CSV file with images: {csv_path}")
elif os.path.exists(csv_path_final):
    csv_path = csv_path_final
    print(f"Using final CSV file: {csv_path}")
else:
    # Look for any CSV files in the exports directory
    csv_files = glob.glob('exports/*.csv')
    if csv_files:
        csv_path = csv_files[0]  # Use the first CSV file found
        print(f"Using available CSV file: {csv_path}")
    else:
