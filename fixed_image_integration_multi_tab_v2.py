...
def upload_to_sheets_multi_tab(df_with_drive_urls):
    try:
        ...
        for platform_name in platforms:
            platform_df = df_with_drive_urls[df_with_drive_urls["Platform"] == platform_name].copy() # Use .copy() to avoid SettingWithCopyWarning
            ...
            try:
                ...
                # Get current sheet data to append correctly
                result = service.spreadsheets().values().get(spreadsheetId=sheet_id, range=f"{sheet_name}!A:A").execute() # Check first column for last row
                values = result.get("values", [])
                start_row = len(values) + 1
                
                # If sheet was just created and headers added, start_row should be 2
                if start_row == 1 and sheet_name not in sheets: # Sheet was empty, headers not yet written by this logic path
                     service.spreadsheets().values().update(
                        spreadsheetId=sheet_id, range=f"{sheet_name}!A1", valueInputOption="RAW", 
                        body={"values": [expected_headers]}).execute()
                     print(f"Added headers to {sheet_name} (first time)")
                     start_row = 2
                elif start_row == 1 and sheet_name in sheets and not values: # Sheet exists but is empty
                    service.spreadsheets().values().update(
                        spreadsheetId=sheet_id, range=f"{sheet_name}!A1", valueInputOption="RAW", 
                        body={"values": [expected_headers]}).execute()
                ...
                # Upload rows sequentially for this platform
                for i, record_tuple in enumerate(platform_df.iterrows()):
                    index, record = record_tuple
                    timestamp = datetime.now().isoformat()
                    platform_df.loc[index, "LastUpdated"] = timestamp # Update LastUpdated in the DataFrame copy
                    row_values = [[platform_df.loc[index, col] for col in expected_headers]]
                    ...
                    time.sleep(0.5) # Zapier compatibility delay
...
if __name__ == "__main__":
    # 1. Ensure images exist for all posts and update CSV
    df_with_image_paths = ensure_images_for_all_posts()
    
    if not df_with_image_paths.empty:
        # 2. Upload these images to Google Drive and get their URLs
        drive_urls_map = upload_to_google_drive(df_with_image_paths)
        
        # 3. Add DriveImageURL column to the DataFrame
        # Create a copy to avoid SettingWithCopyWarning when adding the new column
        df_for_sheets = df_with_image_paths.copy()
        df_for_sheets["DriveImageURL"] = df_for_sheets["ImagePath"].map(drive_urls_map).fillna("")
        
        # 4. Upload the DataFrame to Google Sheets, separated by platform tabs
        upload_to_sheets_multi_tab(df_for_sheets)
    else:
        print("No data to process after ensuring images.")
